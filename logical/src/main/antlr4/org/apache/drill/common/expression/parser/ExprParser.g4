...
schemaCall returns [LogicalExpression e]
	@init{
  	boolean nullability = true;
	}
  : OParen expression dataType
   (T_Not T_Null { nullability = false; } )?
   CParen
  {
    $e = new SchemaExpression($expression.e, $dataType.type, nullability);
  };

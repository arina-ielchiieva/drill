...
public class DrillResultSetTest extends DrillTest {
  ...
  public void test() throws Exception {
    ...
    String query = "select cast(convert_to(types[1].minor[0].valueHolder, 'JSON') as varchar(40)) from " +
        "dfs.`F:\\git_repo\\drill-test-framework\\framework\\resources\\Datasources\\convert\\vvtypes.json`";

    String limit = "select * from (select cast(convert_to(types[1].minor[0].valueHolder, 'JSON') as varchar(40)) from " +
        "dfs.`F:\\git_repo\\drill-test-framework\\framework\\resources\\Datasources\\convert\\vvtypes.json`) t " +
        "limit 0";

    String limit2 = "select * from (select convert_to(types[1].minor[0].valueHolder, 'JSON') from " +
        "dfs.`F:\\git_repo\\drill-test-framework\\framework\\resources\\Datasources\\convert\\vvtypes.json`) t " +
        "limit 0";
    ...
    System.out.println(resultSet.getMetaData().getColumnDisplaySize(1));
    System.out.println("LIMIT 0 without cast");
    resultSet = statement.executeQuery(limit2);
    System.out.println(resultSet.getMetaData().getColumnDisplaySize(1));

...
public class TestCTTAS extends BaseTestQuery {
  ...
  public void testCreateTableWithDifferentStorageFormats() throws Exception {
    ...
    try {
      for (String storageFormat : storageFormats) {
        // check there no other temp tables
  ...
  private static String getSessionId() throws Exception {
    List<QueryDataBatch> queryDataBatches = testSqlWithResults("select session_id as session_id from (values(1))");
    final QueryDataBatch batch = queryDataBatches.get(0);
    final RecordBatchLoader loader = new RecordBatchLoader(getDrillbitContext().getAllocator());
    loader.load(batch.getHeader().getDef(), batch.getData());

    final VectorWrapper schemaValueVector = loader.getValueAccessorById(
        VarCharVector.class,
        loader.getValueVectorId(SchemaPath.getCompoundPath("session_id")).getFieldIds());
    String sessionId = schemaValueVector.getValueVector().getAccessor().getObject(0).toString();
    batch.release();
    loader.clear();
    return sessionId;
  }

  private void cleanSessionDirectory() throws IOException {
    File directory = new File(dirTestWatcher.getDfsTestTmpDir().getAbsolutePath(), sessionId);
    if (directory.exists()) {
  ...
  private void checkPermission() throws IOException {
    java.nio.file.Path sessionTempLocation = Paths.get(dirTestWatcher.getDfsTestTmpDir().getAbsolutePath(), sessionId);
    Optional<java.nio.file.Path> path = Files.list(sessionTempLocation).findFirst();
    assertTrue(path.isPresent());
    Path tmpTablePath = new Path(path.get().toString());

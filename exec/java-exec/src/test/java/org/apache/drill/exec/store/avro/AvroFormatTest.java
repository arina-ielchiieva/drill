...
public class AvroFormatTest extends ClusterTest {
  ...
  public void testMapSchema_withNullValues() throws Exception {
    ...
    queryBuilder().sql(sql, file).print(); //todo no result set testing

    testBuilder()
      .sqlQuery(sql, file)
      .unOrdered()
      .baselineColumns("a_string", "k1", "k2")
      .baselineValues("a_4", "nested_1_string_2", 4)
  ...
  @Test //todo need to verify results
  public void testMapSchemaValueInFilter2() throws Exception {
    ...
    int elementIndex = 1;
    int startRecord = 2;
    int filterValue = 3 * (elementIndex + 2);
    TestBuilder testBuilder = testBuilder()
      .sqlQuery(sql, elementIndex, mapTableName, elementIndex, filterValue)
      .unOrdered()
      .baselineColumns("val");

    for (int i = startRecord + 1; i < AvroDataGenerator.RECORD_COUNT; i++) {
      testBuilder.baselineValues((i + 1) * (elementIndex + 2));
    }
    testBuilder.go();
  }

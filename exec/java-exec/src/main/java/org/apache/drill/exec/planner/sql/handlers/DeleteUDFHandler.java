...
public class DeleteUDFHandler extends DefaultSqlHandler {
  ...
  public PhysicalPlan getPlan(SqlNode sqlNode) throws ForemanSetupException, IOException {
    ...
    UserProtos.StringList list = UserProtos.StringList.newBuilder().addList(jarName).build();
    ...
    try {
      for (CoordinationProtos.DrillbitEndpoint endpoint : context.getActiveEndpoints()) {
        final DrillRpcFuture<UserProtos.StringList> deleteUdf = context.getController().getTunnel(endpoint).deleteUDF(list);

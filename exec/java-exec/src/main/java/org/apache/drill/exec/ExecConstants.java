...
public final class ExecConstants {
  ...
  public static final StringValidator PARQUET_READER_STRINGS_SIGNED_MIN_MAX_VALIDATOR = new StringValidator(PARQUET_READER_STRINGS_SIGNED_MIN_MAX,
    new OptionDescription("Allows binary statistics usage for files created prior to 1.9.1 parquet library version where " +
      "statistics was calculated using incorrectly for UTF-8 data. For cases when user exactly knows " +
      "that data in binary columns in ASCII (not UTF-8), turning this property to 'true' " +
      "enables statistics usage for varchar and decimal types. Default is unset."));

...
public class ClassCompilerSelector {
  ...
  byte[][] getClassByteCode(ClassNames className, String sourceCode)
    ...
    AbstractClassCompiler compiler = getCompiler(sourceCode);
    byte[][] bc = compiler.getClassByteCode(className, sourceCode);

    // Uncomment the following to save the generated byte codes.
    final String baseDir = System.getProperty("java.io.tmpdir") + File.separator + compiler.getClass().getSimpleName();

...
public class InsertHandler extends DefaultSqlHandler {
  ...
  public PhysicalPlan getPlan(SqlNode sqlNode) throws ValidationException, RelConversionException, IOException, ForemanSetupException {
    ...
    SqlIdentifier table = (SqlIdentifier) operands[1];
    final String newTblName = table.names.get(table.names.size() - 1);
    final List<String> schema = table.names.subList(0, table.names.size() - 1);

    final ConvertedRelNode convertedRelNode = validateAndConvert(operands[2]);
    final RelDataType validatedRowType = convertedRelNode.getValidatedRowType();
    final RelNode queryRelNode = convertedRelNode.getConvertedNode();

    List<String> targetColumnList = Lists.newArrayList();
    for (SqlNode node : ((SqlNodeList) operands[3]).getList()) {
      targetColumnList.add(node.toString());
    }

    final RelNode newTblRelNode =
        SqlHandlerUtil.resolveNewTableRel(false, targetColumnList, validatedRowType, queryRelNode);
    ...
    if (SqlHandlerUtil.getTableFromSchema(drillSchema, newTblName) == null) {
      throw UserException.validationError()
          .message("A table or view with given name [%s] does not  in schema [%s]", newTblName, schemaPath)
          .build(logger);
    }

    List<String> partitionColumns =  Lists.newArrayList();

    final RelNode newTblRelNodeWithPCol = SqlHandlerUtil.qualifyPartitionCol(newTblRelNode, partitionColumns);

    log("Calcite", newTblRelNodeWithPCol, logger, null);

    // Convert the query to Drill Logical plan and insert a writer operator on top.
    DrillRel drel = convertToDrel(newTblRelNodeWithPCol, drillSchema, newTblName, partitionColumns, newTblRelNode.getRowType());
    Prel prel = convertToPrel(drel, newTblRelNode.getRowType(), partitionColumns);

...
public class UDFHandler {
  ...
  public StringList createUDF(UDFHolder holder) {
    ...
    File binary = new File(pathToClasspath, holder.getBinary().getName());
    File source = new File(pathToClasspath, holder.getSource().getName());
    ...
    try {
      FileUtils.writeByteArrayToFile(binary, holder.getBinary().toByteArray());
      FileUtils.writeByteArrayToFile(source, holder.getSource().toByteArray());
      ...
      //todo throw new RpcException(e);
      throw new RuntimeException(e);
    }
  }

  public StringList deleteUDF(JarHolder holder) {
    // get binary and source jar name
    String binaryJarName = holder.getName();

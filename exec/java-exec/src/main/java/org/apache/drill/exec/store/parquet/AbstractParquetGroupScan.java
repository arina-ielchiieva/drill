...
public abstract class AbstractParquetGroupScan extends AbstractFileGroupScan {
  ...
  //todo real filter for partition pruning to apply during page pruning
  ...
  public GroupScan applyLimit(int maxRecords) {
    ...
    Set<String> filePaths = new HashSet<>(); // HashSet keeps a filePath unique.
    for (RowGroupInfo rowGroupInfo : rowGroupInfos.subList(0, index)) {
      filePaths.add(rowGroupInfo.getPath());
    }
  ...
  public void modifyFileSelection(FileSelection selection) {
    ...
    for (String filePath : files) {
      entries.add(new ReadEntryWithPath(filePath));
    }

    List<RowGroupInfo> newRowGroupList = new ArrayList<>();
    for (RowGroupInfo rowGroupInfo : rowGroupInfos) {
      if (fileSet.contains(rowGroupInfo.getPath())) {
        newRowGroupList.add(rowGroupInfo);
      }
    }
    rowGroupInfos = newRowGroupList;
  }

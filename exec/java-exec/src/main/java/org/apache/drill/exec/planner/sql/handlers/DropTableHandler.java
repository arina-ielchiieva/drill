...
public class DropTableHandler extends DefaultSqlHandler {
  ...
  private boolean removeTemporaryTable(SchemaPlus defaultSchema, List<String> tableSchema, String tableName) {
    ...
    final String temporaryTable = context.getSession().findTemporaryTable(tableName);
    ...
    final Table tableToDrop = SqlHandlerUtil.getTableFromSchema(drillSchema, temporaryTable);
    if (tableToDrop != null && tableToDrop.getJdbcTableType() == Schema.TableType.TABLE) {
      try {

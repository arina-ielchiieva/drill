...
public class UserSession implements Closeable {
  ...
  /** Stores list of session temporary locations, path to location, value is file system associated with location. **/
   ...
   * If original table name already exist, new name is not regenerated and is reused.
   * This can happen in default temporary workspace was changed (file system or location) or
   * orphan temporary table name remained (name was registered but table creation did not succeed).
   *
  ...
  public String registerTemporaryTable(AbstractSchema schema, String tableName) throws IOException {
    if (schema instanceof WorkspaceSchemaFactory.WorkspaceSchema) {
      ...
      final String temporaryTableName = Paths.get(sessionId, UUID.randomUUID().toString()).toString();
      final String oldTemporaryTableName = temporaryTables.putIfAbsent(tableName, temporaryTableName);
  ...
  private void addTemporaryLocation(WorkspaceSchemaFactory.WorkspaceSchema temporaryWorkspace) throws IOException {
    final DrillFileSystem fs = temporaryWorkspace.getFS();
    final Path temporaryLocation = new Path(Paths.get(fs.getUri().toString(),
        temporaryWorkspace.getDefaultLocation(), sessionId).toString());

    final FileSystem fileSystem = temporaryLocations.putIfAbsent(temporaryLocation, fs);

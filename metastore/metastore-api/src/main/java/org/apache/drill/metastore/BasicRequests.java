...
/**
 * Provides handy methods to retrieve Metastore data for analysis.
 * Contains list of most frequent requests to the Metastore without need
 * to write filters and transformers from {@link MetadataUnit} class.
 */
public class BasicRequests {
  ...
  public BasicRequests(Metastore metastore) {
  ...
  public MetastoreTableInfo metastoreTableInfo(TableInfo tableInfo) {
    ...
    long version = metastore.metadata().version();
    MetadataUnit unit = retrieveSingleElement(request(requestMetadata));
  ...
  public boolean hasMetastoreTableInfoChanged(MetastoreTableInfo metastoreTableInfo) {
    if (metastore.metadata().supportsVersioning()
      && metastoreTableInfo.metastoreVersion() == metastore.metadata().version()) {
      return false;
    }
  ...
  public List<BaseTableMetadata> tablesMetadata(FilterExpression filter) {
    RequestMetadata requestMetadata = RequestMetadata.builder()
      ...
      .metadataKey(MetadataInfo.GENERAL_INFO_KEY)
      .metadataType(MetadataType.TABLE.name())
      .requestColumns(MetadataUnit.SCHEMA.tableColumns())
      .build();

    List<MetadataUnit> units = request(requestMetadata);
    return BasicTransformer.tables(units);
  }
  ...
  public List<SegmentMetadata> segmentsMetadataByMetadataKey(TableInfo tableInfo, List<String> locations, String metadataKey) {
    RequestMetadata requestMetadata = RequestMetadata.builder()
      ...
      .requestColumns(MetadataUnit.SCHEMA.segmentColumns())
      .build();

    List<MetadataUnit> units = request(requestMetadata);
    return BasicTransformer.segments(units);
  }
  ...
  public List<PartitionMetadata> partitionsMetadata(TableInfo tableInfo, List<String> metadataKeys, String column) {
    RequestMetadata requestMetadata = RequestMetadata.builder()
      ...
      .requestColumns(MetadataUnit.SCHEMA.partitionColumns())
      .build();

    List<MetadataUnit> units = request(requestMetadata);
    return BasicTransformer.partitions(units);
  }
  ...
  public List<FileMetadata> filesMetadata(TableInfo tableInfo, String metadataKey, List<String> paths) {
    RequestMetadata requestMetadata = RequestMetadata.builder()
      ...
      .requestColumns(MetadataUnit.SCHEMA.fileColumns())
      .build();

    List<MetadataUnit> units = request(requestMetadata);
    return BasicTransformer.files(units);
  }
  ...
  public FileMetadata fileMetadata(TableInfo tableInfo, String metadataKey, String path) {
    ...
    List<MetadataUnit> units = request(requestMetadata);
    return retrieveSingleElement(BasicTransformer.files(units));
  }
  ...
  public List<RowGroupMetadata> rowGroupsMetadata(TableInfo tableInfo, String metadataKey, String path) {
   RequestMetadata requestMetadata = RequestMetadata.builder()
      ...
      .requestColumns(MetadataUnit.SCHEMA.rowGroupColumns())
      .build();

    List<MetadataUnit> units = request(requestMetadata);
    return BasicTransformer.rowGroups(units);
  }
   ...
   * @return list of segments / files / rows groups metadata in {@link BasicTransformer.MetadataHolder} instance
   */
  public BasicTransformer.MetadataHolder fullSegmentsMetadataWithoutPartitions(TableInfo tableInfo, List<String> metadataKeys, List<String> locations) {
    RequestMetadata requestMetadata = RequestMetadata.builder()
      .tableInfo(tableInfo)
      .metadataKeys(metadataKeys)
      .locations(locations)
    ...
    List<MetadataUnit> units = request(requestMetadata);
    return BasicTransformer.all(units);
  }
   ...
   *
   * @param tableInfo table information
   * @return {@link MetadataUnit} instance with set interesting columns and partition keys if present
   */
  public MetadataUnit interestingColumnsAndPartitionKeys(TableInfo tableInfo) {
    RequestMetadata requestMetadata = RequestMetadata.builder()
      .tableInfo(tableInfo)
      .metadataKey(MetadataInfo.GENERAL_INFO_KEY)
      .metadataType(MetadataType.TABLE.name())
  ...
  /**
   * Executes Metastore read request based on given information in {@link RequestMetadata}.
   *
   * @param requestMetadata request metadata
   * @return list of metadata units
   */
  public List<MetadataUnit> request(RequestMetadata requestMetadata) {
  ...
  public static class RequestMetadata {
    ...
    public static class Builder {
      ...
      /**
       * Create filter based on given parameters and adds to the given list of filters.
